...
/** \ingroup Enum
 * Defines the connection configuration codes provided to the LeapOpenConnection()
 * function inside a LEAP_CONNECTION_CONFIG struct. Currently there are no useful
 * codes.
 * @since 3.0.0
 */
typedef enum _eLeapConnectionFlags {
  eLeapConnectionFlags_Default          = 0x00000000,
} eLeapConnectionFlags;
...
enum eLeapDeviceCaps {
  ...
  /** The device is embedded in another piece of hardware, such as a keyboard or laptop. @since 3.0.0 */
 ...
 */
typedef enum _eLeapDeviceType {
  /** An unknown device type value. @since 3.1.3 */
  eLeapDeviceType_Unknown         = 0x0000,

  /** The Leap Motion consumer peripheral. @since 3.0.0 */
  eLeapDeviceType_Peripheral      = 0x0003,

  /** HOPS/PONGO Legacy device. @since 3.0.0 */
  eLeapDeviceType_Legacy          = 0x1001,

  /** Leap Motion HOPS Keyboard. @since 3.0.0 */
  eLeapDeviceType_Hops            = 0x1002,

  /** Leap Motion Pongo laptop-embedded device. @since 3.0.0 */
  ...
  eLeapDeviceType_Invalid = 0xFFFFFFFF
} eLeapDeviceType;
...
typedef struct _LEAP_DEVICE_INFO {
  ...
  /** One of the LEAP_DEVICE_TYPE members. @since 3.0.0 */
...
/** \ingroup Structs
 * Describes the digit of a handType.
...
typedef struct _LEAP_DIGIT {
  ...
  union {
    ...
    struct {
      /**
       * The finger bone wholly inside the handType.
...
/** \ingroup Structs
 * Properties associated with the palm of the handType.
...
typedef struct _LEAP_PALM {
  ...
  /**
   * The normal vector to the palm. If your handType is flat, this vector will
  ...
  /**
   * The estimated width of the palm when the handType is in a flat position.
   * @since 3.0.0
   */
  float width;
...
typedef enum _eLeapHandType {
  /** A left handType. @since 3.0.0 */
  eLeapHandType_Left,

  /** A right handType. @since 3.0.0 */
  eLeapHandType_Right
} eLeapHandType;

/** \ingroup Structs
 * Describes a tracked handType. @since 3.0.0
 */
typedef struct _LEAP_HAND {
  /**
   * A unique ID for a handType tracked across frames.
   * If tracking of a physical handType is lost, a new ID is assigned when
  ...
  /**
   * Identifies the chirality of this handType. @since 3.0.0
   */
  eLeapHandType type;

  /**
   * How confident we are with a given handType pose. Not currently used (always 1.0).
   * @since 3.0.0
   */
  float confidence;

  /**
   * The total amount of time this handType has been tracked, in microseconds.
   * @since 3.0.0
   */
  ...
  /**
   * The normalized estimate of the grab handType pose.
  ...
  /** The fingers of this handType. @since 3.0.0 */
  union {
    struct {
    ...
    /** The fingers of the handType as an array. @since 3.0.0 */
    LEAP_DIGIT digits[5];
  };

  /**
   * The arm to which this handType is attached.
 ...
 * frames.
 *
 * You are responsible for allocating a buffer large enough to hold the data of the frame.
...
/** \ingroup Functions
 * Provides the human-readable canonical name of the specified device type.
 *
 * This method is guaranteed to never return null for the LEAP_DEVICE_INFO.type field
 * returned by a successful call to LeapGetDeviceInfo
 *
 * @param type The type of the device
 * @returns The string name of the device, or null if the device type string is invalid.
 */
LEAP_EXPORT const char* LEAP_CALL LeapDeviceTypeToString(eLeapDeviceType type);
